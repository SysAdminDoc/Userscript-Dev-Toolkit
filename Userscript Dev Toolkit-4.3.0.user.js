...
(function() {
    ...
    const createElement = (tag, classes = [], attributes = {}, text = '') => {
        ...
        el.setAttribute('data-devtoolkit', 'true'); // Mark element as part of the toolkit
        return el;
    };
    ...
    class DevToolkit {
        constructor() {
            ...
            // Define all available components for the toolkit
        ...
        async init() {
            ...
            this.bindCoreEvents();
            this.log(`Dev Toolkit v4.3.0 Initialized.`);
        }
        ...
        injectCoreMarkup() {
            ...
            // Lips for showing the panel after it has been collapsed
        ...
        injectStyles() {
            GM_addStyle(`
                ...
                .element-highlight { outline: 2px solid #ff00ff !important; box-shadow: 0 0 10px #ff00ff !important; background-color: rgba(255, 0, 255, 0.2) !important; cursor: crosshair !important; }
        ...
        startElementPicker(callback, sourceComponent) {
            ...
            this.log('Element picker started.');
        }
        ...
        stopElementPicker() {
            ...
            // Notify all components that might have a picker to update their button state
            Object.values(this.components).forEach(c => c.updatePickerButton?.(false));
            this.log('Element picker stopped.');
        }

        onPickerHover(e) {
            if (e.target.closest('[data-devtoolkit]')) {
                this.removeHighlight();
                return;
            }
        ...
        onPickerClick(e) {
            ...
            if (this.onElementSelectedCallback) {
                this.onElementSelectedCallback(e.target);
            }
            e.target.scrollIntoView({ behavior: 'smooth', block: 'center' });
            this.log(`Element captured: <${e.target.tagName.toLowerCase()}>`);
            this.stopElementPicker();
        }

        removeHighlight() {
            if (this.lastHighlightedElement) {
                this.lastHighlightedElement.classList.remove('element-highlight');
                this.lastHighlightedElement = null;
            }
        }
    ...
    class ToolkitComponent {
        ...
        bindEvents() {}
        update() {}
        updatePickerButton(isPickerActive) {
            const btn = qs('.element-picker-btn', this.el);
            if (btn) {
                btn.textContent = isPickerActive ? 'Cancel Picking' : 'Pick Element';
                ...
                if(tabBtn) {
                    ...
                    if (isPickerActive && !dot) {
                        tabBtn.appendChild(createElement('span', ['picker-active-dot']));
                    } else if (!isPickerActive && dot) {
                        dot.remove();
                    }
                }
            }
    ...
    class InspectorComponent extends ToolkitComponent {
        ...
        update(target) {
            ...
            if (target.className) summary += ` | <strong>Class:</strong> .${target.className.trim().replace(/\s+/g, '.')}`;
            ...
            // Also send the element to the uBlock component for filter generation
    ...
    class uBlockComponent extends ToolkitComponent {
        ...
        getHTML() { return `<h4>uBlock Origin Filter Generator</h4><p>Pick an element to generate cosmetic filters. Click any rule to copy it.</p><button class="toolkit-btn primary element-picker-btn">Pick Element</button><div class="ublock-output" style="margin-top: 15px;" data-placeholder="No element selected."></div>`; }
        bindEvents() { qs('.element-picker-btn', this.el).addEventListener('click', () => { this.toolkit.isInspectorActive ? this.toolkit.stopElementPicker() : this.toolkit.startElementPicker(target => this.update(target), this); }); }
        update(target) {
            if (!this.el || !target) return;
        ...
        generateFilters(el) {
            ...
            if (el.id) {
                filters.push({ desc: 'By ID (strongest)', rule: `${domain}##${tag}#${el.id}` });
            }

            // Class-based
            const classes = Array.from(el.classList).filter(c => !c.startsWith('element-highlight'));
            if (classes.length > 0) {
                filters.push({ desc: 'By Classes', rule: `${domain}##${tag}.${classes.join('.')}` });
            }
            ...
            for (const attr of uniqueAttrs) {
                if (el.hasAttribute(attr)) {
                    filters.push({ desc: `By Attribute [${attr}]`, rule: `${domain}##${tag}[${attr}="${el.getAttribute(attr)}"]` });
                }
            }

            // Text-based (if it has direct, non-empty text content)
            ...
            if (directText) {
                ...
                if (text) {
                     filters.push({ desc: 'By Text Content', rule: `${domain}##${tag}:has-text(/${text.replace(/\//g, '\\/')}/)`});
                }
            }
            ...
            if (el.parentElement && el.parentElement !== document.body) {
                 ...
                 if (parent.id) {
                      filters.push({ desc: 'By Parent ID', rule: `${domain}##${parentTag}#${parent.id} > ${tag}` });
                 } else if (parent.className) {
                     const parentClasses = Array.from(parent.classList).join('.');
                     if (parentClasses) {
                         filters.push({ desc: 'By Parent Class', rule: `${domain}##${parentTag}.${parentClasses} > ${tag}` });
                     }
                 }
            }

            if (filters.length === 0) {
                 filters.push({ desc: 'Generic fallback', rule: `${domain}##${getCssPath(el)}` });
            }
            return filters;
        }
    }
    ...
    class CSSViewerComponent extends ToolkitComponent {
        constructor(toolkit) { super(toolkit, 'css-viewer', '🎨 CSS'); this.rawCss = ''; }
        getHTML() { return `<h4>Page Stylesheet Viewer</h4><p>Fetch all loaded CSS from external and inline styles. Junk rules can be removed automatically.</p><div style="display:flex; gap: 8px; margin-bottom: 10px;"><button id="fetch-styles-btn" class="toolkit-btn primary" style="flex:1;">Fetch All Styles</button><button id="copy-styles-btn" class="toolkit-btn" style="display: none;">Copy</button><button id="export-styles-btn" class="toolkit-btn" style="display: none;">Download .css</button></div><div class="setting-row" style="font-size: 0.9em;"><label>Remove junk/empty styles</label><label class="toggle-switch"><input type="checkbox" id="clean-css-toggle" checked><span class="toggle-slider"></span></label></div><pre id="styles-log-box" class="log-box" data-placeholder="Click 'Fetch' to load styles." style="max-height: none; flex-grow: 1;"></pre>`; }
        bindEvents() {
            qs('#fetch-styles-btn', this.el).addEventListener('click', () => this.fetchAllStyles());
            qs('#export-styles-btn', this.el).addEventListener('click', () => { const css = qs('#styles-log-box', this.el).textContent; GM_download(css, `page-styles-${window.location.hostname}.css`, 'text/css'); this.toolkit.log('CSS download initiated.'); });
            qs('#copy-styles-btn', this.el).addEventListener('click', () => { const css = qs('#styles-log-box', this.el).textContent; GM_setClipboard(css); this.toolkit.log('All fetched CSS copied.'); });
        ...
        async fetchAllStyles() {
            ...
            this.toolkit.log('Starting stylesheet fetch...');
            ...
            for (const sheet of styleSheets) {
                if (sheet.href) {
                    promises.push(new Promise(resolve => {
                        GM_xmlhttpRequest({
                            method: 'GET', url: sheet.href,
                            onload: r => { allCss += `/* === From ${sheet.href} === */\n${r.responseText}\n\n`; resolve(); },
                            onerror: () => { allCss += `/* FAILED TO FETCH: ${sheet.href} */\n\n`; resolve(); }
                        });
                    }));
                } else {
                    try {
                        allCss += `/* === Inline <style> tag in document === */\n${Array.from(sheet.cssRules).map(r => r.cssText).join('\n')}\n\n`;
                    } catch (e) {
                        allCss += `/* === Could not read inline <style> due to CORS policy === */\n\n`;
                    }
                }
            }
            ...
            this.toolkit.log('Style fetching complete.');
        }
        cleanCss(cssText) {
            // Remove multi-line comments, but keep our header
            cssText = cssText.replace(/\/\*(?! TOOLKIT_DUMP_HEADER)[\s\S]*?\*\//g, '');
            // Remove single-line comments (less common in CSS but good to have)
    ...
    class DebuggerComponent extends ToolkitComponent {
        ...
        getHTML() { return `<h4>JavaScript Debugger</h4><p>This button will trigger a <code>debugger;</code> statement. The browser's developer tools must be open for it to have an effect.</p><button id="trigger-debugger-btn" class="toolkit-btn primary">Trigger Debugger</button><p style="font-size:0.9em; opacity:0.7; margin-top: 15px;">Use this to pause script execution at a specific point and inspect the call stack, scope, and variables, which is invaluable for debugging complex logic in your userscripts.</p>`; }
        bindEvents() {
            qs('#trigger-debugger-btn', this.el).addEventListener('click', () => {
                this.toolkit.log('Debugger triggered. Check your browser\'s developer console.');
                // This is the correct way to use the debugger statement.
                // It pauses execution only if the developer tools are open.
                debugger;
            });
        }
    }
    ...
    class FunctionGuideComponent extends ToolkitComponent {
        ...
        getHTML() { return `<h4>How to Use This Toolkit</h4> <p>A brief guide to the toolkit's features.</p> <details> <summary><strong>🔍 Inspector</strong></summary> <ul> <li><strong>What it does:</strong> Lets you click any element on a page to get its technical details.</li> <li><strong>Why it's useful:</strong> To get a reliable "selector" (address) for an element you want to modify, hide, or interact with.</li> </ul> </details> <details> <summary><strong>🛡️ uBlock</strong></summary> <ul> <li><strong>What it does:</strong> Generates advanced filter rules for any element you pick.</li> <li><strong>Why it's useful:</strong> The fastest way to create cosmetic filters to block annoying elements, using various strategies like attributes, text, or parent selectors.</li> </ul> </details> <details> <summary><strong>🎨 CSS Viewer</strong></summary> <ul> <li><strong>What it does:</strong> Scans the page and pulls all CSS code into one place.</li> <li><strong>Why it's useful:</strong> For understanding how a site is styled, finding specific rules, and downloading a cleaned-up version for your own use.</li> </ul> </details> <details> <summary><strong>🐛 Debugger</strong></summary> <ul> <li><strong>What it does:</strong> Pauses script execution when clicked (if DevTools is open).</li> <li><strong>Why it's useful:</strong> Allows you to inspect variables and the call stack at the exact moment you trigger it, which is essential for complex debugging.</li> </ul> </details> <details> <summary><strong>📋 Console</strong></summary> <ul> <li><strong>What it does:</strong> Shows logs from this toolkit and errors specifically from userscripts, ignoring website noise.</li> <li><strong>Why it's useful:</strong> The first place to check if your script has errors.</li> </ul> </details>`; }
    }

    // --- SETTINGS & UTILITIES ---

    class SettingsManager {
        constructor(toolkit) { this.toolkit = toolkit; this.prefs = {}; this.STORAGE_KEY = 'dev-toolkit-prefs-v4.3'; }
        ...
        async load() { const defaults = this.getDefaults(); const loadedPrefs = await GM_getValue(this.STORAGE_KEY, {}); this.prefs = { ...defaults, ...loadedPrefs, ui: { ...defaults.ui, ...(loadedPrefs.ui || {}) }, components: { ...defaults.components, ...(loadedPrefs.components || {}) }, debug: { ...defaults.debug, ...(loadedPrefs.debug || {}) } }; }
